...
const WorldMap = () => {
  ...
  useEffect(() => {
    if (selectedProducto && selectedPerfil && selectedModalidad) {
      ...
      fetch(`/api/zonas?cod_producto_comercial=${codProductoComercial}&cod_modalidad=${codModalidad}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  useEffect(() => {
    if (selectedZona) {
      const codZona = selectedZona.value;
      fetch(`/api/paises?cod_zona_tarif=${codZona}`)
        .then((res) => res.json())
        .then((data) => {
